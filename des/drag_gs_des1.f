...
! Particle volume.
      use discretelement, only: PVOL, PART_VOL_INTERSEC, PART_CELLS
...
! Gas pressure force.
            ...
            ENDDO
         ELSE
            DO LC =2, PART_CELLS(NP,1) + 1
               IJK = PART_CELLS(NP,LC)
               WEIGHT = PART_VOL_INTERSEC(IJK,NP)/PVOL(NP)
               lEPG = lEPG + EP_G(IJK)*WEIGHT
               VELFP(1) = VELFP(1) + UGC(IJK)*WEIGHT
               VELFP(2) = VELFP(2) + VGC(IJK)*WEIGHT
               VELFP(3) = VELFP(3) + WGC(IJK)*WEIGHT
...
! This avoids FP exceptions for some ghost particles.
         ...
         ELSE
            DO LC =2,PART_CELLS(NP,1)+1
               IJK = PART_CELLS(NP,LC)
               WEIGHT = PART_VOL_INTERSEC(IJK,NP)/PVOL(NP)/VOL(IJK)
