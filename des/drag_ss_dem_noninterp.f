...
! The count and a list of particles in IJK
      use discretelement, only: PINC, PIC, MAX_PIP
...
! Particle radius and volume.
      use discretelement, only: DES_RADIUS, PVOL, PART_VOL_INTERSEC
...
! skipping indices that do not represent particles and ghost particles
            ...
            IF(PEA(NP,4)) CYCLE
            IF(PART_VOL_INTERSEC(IJK,NP) == 0.0d0) CYCLE
            FRAC_INT = PART_VOL_INTERSEC(IJK,NP)/PVOL(NP)
...
! Calculating the accumulated solids-solids drag force.
            ...
            FC(:,NP) = FC(:,NP) + D_FORCE(:)*PVOL(NP)*FRAC_INT
