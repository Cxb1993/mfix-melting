...
!---------------------------------------------------------------------//
      ...
      DO IJK = IJKSTART3, IJKEND3
         ...
         R_sp(IJK,1,:) = R_sp(IJK,1,:) + &
            DES_R_sp_TFM(IJK,:)/DEM_to_TFM
         ...
         WHERE(X_s(IJK,1,:) > SMALL_NUMBER)
            RoX_sc(IJK,1,:) = RoX_sc(IJK,1,:) + &
               DES_R_sc_TFM(IJK,:) / (DEM_to_TFM * X_s(IJK,1,:))
         ELSEWHERE
            RoX_sc(IJK,1,:) = RoX_sc(IJK,1,:) + 1.0d-9/DEM_to_TFM
         ENDWHERE
         ...
         SUM_R_s(IJK,1) = SUM_R_s(IJK,1) + &
            DES_SUM_R_s_TFM(IJK) / DEM_to_TFM

         HOR_g(IJK) = HOR_g(IJK) + &
            DES_HOR_g(IJK) / DEM_to_TFM
			
		 HOR_s(IJK,1) = HOR_s(IJK,1) + &
		    DES_HOR_s(IJK) / DEM_to_TFM

         !print *, "Cell ", IJK, SUM_R_s(IJK,1)
         !print *, "Cell ", IJK, ROP_s(IJK,1)
      ENDDO
